...
<!--#julen: creo que deberiamos añadir la ref/autora de esta figura-->
...
# julen: me da bastante toc la almuadilla sin espacio despues. el ctrl+r añade un espacio. tu a veces lo haces, a veces no... lo podrias añadir para ser consistentes porfa?
...
<!--# julen: en el 3 creo que no esta claro lo que tienen que hacer. podrias aclararlo?-->
...
<!--# Jul: tu siempre dices que es mejor usar el pipe de Rbase, por qué? No sería más consistente usar el de tidiverse? julen: el pipe de antes no era de tidyverse sino de magritt por lo que estariamos añadiendo una dependencia más https://magrittr.tidyverse.org/reference/pipe.html. ademas no permite algunas opciones que daba el de magritt, que creo que mejoran el codigo y son consistentes con los cambios que se han hecho en tidyverse (e.g. en purrr tener que asignar nombres a los argumentos en vez de utilizar .) https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/-->
...
# julen: despues del pipe añadirira un salto de linea siempre
#Filter
penguins |> filter(island == "Dream") 
penguins |> filter(island == "Dream", body_mass_g > 4500) #Se pueden combinar criterios
penguins |> filter(island == "Dream|Chinstrap")


#Select
penguins |> select(sex, body_mass_g)
penguins |> select(-species)
penguins |> select(ends_with("mm")) #Seleccionar variables que tienen un patrón
penguins |> select(ends_with("mm|g")) #Seleccionar variables que tienen un patrón

#Poner enlace a cheatsheet the string

penguins |> select(sex, body_mass_g, everything()) #Se puede usar para reordenar variables

#Arrange
penguins |> select(sex, body_mass_g) |> arrange(body_mass_g)
penguins |> select(sex, body_mass_g) |> arrange(desc(body_mass_g))


#Mutate
penguins |> 
  mutate(bill_volume = (bill_length_mm * bill_depth_mm)/2) |> 
  select(bill_volume, everything())

penguins |> 
  mutate(female_penguin = case_when(sex == "female" & body_mass_g < 3600 ~ "small female",
    sex == "female" & body_mass_g >= 3600 ~ "big female",
    TRUE ~ NA)) |> 
  select(female_penguin, sex, body_mass_g)

#Summarise
#Con funciones que resumen: n, n_distinct, mean, etc.
...
#Añadir leer y guardar

```

Otra diferencia fundamental entre Rbase y tidyverse importante para avanzar en este curso es que el *name masking* es diferente. En R, si un argumento no está definido en una función, R buscará ese nombre en el nivel del entorno inmediatamente superior. En general, con Rbase sólo podemos llamar a objetos que forman parte del environment (env-variables); en cambio, con tidyverse se puede llamar también a las variables dentro de las tablas (data-variables). Esta característica simplifica el código en tidyverse, pero la ventaja no tiene coste cero y tiene implicaciones en la gramática de las funciones como veremos más adelante.

<!--# Quiero desarrollarlo mas en la parte de escribir funciones -->
...
<!--# julen: este ejercicio no funciona porque no existe la variable plots. le he puesto eval: false por ahora. Y que significa TBD? tambien le daria nombre al chunk. esta un poco guarrete no? que quieres hacer aqui?-->
...
# Varias opciones
# julen: no ibas a cambiar el nombre de esta funcion??
