...
In the example above, the length 4 numeric vector `c(1, 2, 3, 4)` (object) have two names: "a" and "b".
...
## How to write functions {#writefun}
...
<!--# Vero: para mi aquí falta una frase ligando la OOP con los for loops. O sea, me parece que sale un poco de la nada -->

*for loops* and *while loops* are recommended to delve into the world of iterations because they make each iteration very explicit so it is clear what is happening.
...
<!--# Vero: Yo aqui pondria para introducir lo siguiente: Programming a loop needs to define three different parts: the output, the sequence and the body. -->
...
1.  Output: here we determine the output space, i.e., first we have to create the "notebook" where we are going to write down all the results. This is very important for efficiency since if we increase the size of the *for loop* <!--# Vero: seria "of the output" no? -->in each iteration with `c()` or another function that adds elements, the *for loop* will be much slower.
...
R is a functional programming language so it relies primarily on a function-centric <!--# Vero: function-central? --> problem-solving style (<https://adv-r.hadley.nz/fp.html>). A functional is a function that takes a function as input and returns a vector or other type of object as output.
...
To program a functional, first, we solve the problem for an element. Next, we generate a function that allows us to wrap the solution in a function ( <!--# Vero: añadir "what we did in @sec-writefun"- -->). Finally, we *apply the function to all the elements we are interested in.* That is, we divide large problems into smaller problems and solve each task with one or more functions.
...
Iterating over a vector <!--# Vero: es confuso porque no es siempre un vector. Pondría "iterating over one vector (i.e. vector of values or vector of elements in a list)" --> is so common that the {purrr} package provides a family of functions (the `map()` family) for it. Remember that under the hood data frames are lists of vectors of the same length, so any calculation by rows or columns involves iterating over a vector. There is a function in {purrr} for each type of output. The suffixes indicate the type of output we want:
...
`map_*()` is vectorized over one argument, e.g. `(x)`. The function will operate on all elements of `x`, i.e., each value if `x` is a vector, each column if `x` is a `data.frame`, or each element if `x` is a list.
...
# shortcut to generate an anonymous function #VERO: pondria escrito cual es el shortcut
...
<!--# Vero: queda raro poner lo siguiente sin una frase introductoria. PArece que es del ejercicio -->
...
`dplyr::across()` is similar to `map()` but instead of doing something with each element of a vector, data frame or list, it does something with each column in a data frame. <!--# Vero: pero map también hace eso si x es un data.frame no?. Sería "ONLY with each column in a data frame"? -->
...
R base provides three OOP systems (S3 –which is the most used-, S4 and RC), although there are also other OOP systems provided by different CRAN packages.
