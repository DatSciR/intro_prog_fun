...
There is no base R equivalent to `walk()`. You can mimic its behavior by wrapping the result of `lapply()` in `invisible()` or save it to a variable that is never used.

For R base functionals that have no equivalent in purrr, see: <https://adv-r.hadley.nz/functionals.html#base-functionals>
