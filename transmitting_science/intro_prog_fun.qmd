...
Working with lists is very common in R. In fact, the default output of `map()` is lists. However, lists can sometimes become messy — for example, when we lose the name of each list element. The `nest()` function from the `{tidyr}` package allows us to work with list-columns inside data frames, generating one row for each group defined by the non-nested columns.
...
In general, with base R, we can only call objects that are part of the environment (env-variables); however, with *tidyverse*, we can also call variables within tables (data-variables) as objects. This feature called [*non-standard evaluation*](http://adv-r.had.co.nz/Computing-on-the-language.html) simplifies the code in *tidyverse*, but the advantage does not come for free and has implications for the grammar of the functions as we will see later.
...
R is a functional programming language so it relies primarily on a function-centric problem-solving style (<https://adv-r.hadley.nz/fp.html>).
